...
=head1 Usage
  ...
  --eval <str>    set e-value cutoff for blast hits, default=1e-20
...
if ($Step == 1) {
	`makeblastdb -in $pep1_file -dbtype nucl & makeblastdb -in $pep2_file -dbtype nucl & wait`; 
	`blastn -num_threads 10 -evalue $Cutoff -outfmt 6 -db $pep2_file -query $pep1_file -out $Outdir/$pep1_file_basename--$pep2_file_basename.m8 & blastn -num_threads 10 -evalue $Cutoff -outfmt 6 -db $pep1_file -query $pep2_file -out $Outdir/$pep2_file_basename--$pep1_file_basename.m8 & wait`;
